...
class MuninnMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # F3: Offline buffer — when Muninn is down, queue memories here
        self._offline_buffer: List[Dict[str, Any]] = []
        self._offline_lock = threading.Lock()
        self._offline_max = 200  # Max queued memories to prevent RAM bloat
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                # F3: Flush offline buffer if we have queued memories
    ...
    def _do_route(self, text: str, timeout: Optional[float] = None) -> str:
        ...
        try:
            ...
            # F1.4/T3: Fetch recent timeline events for temporal context
            timeline_lines = []
            try:
                tl_resp = self._session.get(
                    "/api/v1/timeline/recent?limit=3",
                    timeout=PING_TIMEOUT,
                )
                if tl_resp.status_code == 200:
                    tl_events = tl_resp.json()
                    if tl_events:
                        timeline_lines.append("  ~~ Timeline reciente ~~")
                        for ev in tl_events:
                            ts = ev.get("timestamp", "")[11:19] if ev.get("timestamp") else ""
                            summary = ev.get("summary", "")[:100]
                            tl = ev.get("type", "")
                            timeline_lines.append(f"     [{ts}] ({tl}) {summary}")
            except Exception:
                pass  # Timeline is optional — don't break prefetch
    ...
    def _sync_turn_worker(self, user_content: str, assistant_content: str,
        ...
        F1.4: Also records milestones in timeline when detected.
        F2.5: Extracts rich memories from the turn for dreaming/consolidation.
        """
        try:
            ...
            # F1.4: Detect milestones in assistant response and record in timeline
            self._maybe_record_timeline_event(user_content, assistant_content, session_id)

            # F2.5: Extract rich memories from the turn
            self._extract_rich_memories(user_content, assistant_content, session_id)

        except Exception as e:
            logger.debug("Muninn sync_turn failed: %s", e)

    # -- F2.5: Rich memory extraction --------------------------------------

    def _extract_rich_memories(self, user_content: str, assistant_content: str,
                                session_id: str = "") -> None:
        """Extract structured memories from the turn for dreaming.

        Instead of just storing the raw event, we extract valuable context:
        - User preferences/corrections (explicit or implicit)
        - Decisions made (checkmark patterns, choices)
        - Errors encountered and how they were resolved
        - Tool results that contain durable facts
        - Project progress (completions, milestones)

        These get stored as individual memories linked to relevant peers.
        The dreaming loop will later consolidate/relevance-rank them.
        """
        ...
        try:
            ...
            # 1. User corrections - when Diego corrects the agent
            correction_patterns = [
                "no,", "no es", "incorrecto", "equivocad", "no quiero",
                "mejor ", "prefiero", "cambi", "en realidad",
                "olvid", "ya te dije", "ya dijimos", "dije que",
            ]
            user_lower = user_content[:300].lower()
            for pat in correction_patterns:
                if pat in user_lower:
                    extracted.append({
                        "content": f"[CORRECCION] {user_content[:200]}",
                        "type": "correccion",
                        "peer_ids": ["peer_usuario"],
                        "confidence": 0.8,
                    })
                    break  # One correction memory per turn is enough

            # 2. Decisions and completions
            decision_patterns = [
                "funcionando", "arreglado",
                "implementad", "cread", "migrad", "desplegad",
            ]
            asst_lower = assistant_content[:500].lower()
            for pat in decision_patterns:
                if pat in asst_lower:
                    # Extract the line containing the completion
                    for line in assistant_content[:500].split("\n"):
                        if pat in line.lower() and len(line.strip()) > 10:
                            extracted.append({
                                "content": f"[DECISION/AVANCE] {line.strip()[:200]}",
                                "type": "decision",
                                "peer_ids": [],
                                "confidence": 0.7,
                            })
                            break
                    break  # One decision memory per turn

            # 3. Error resolutions - when an error was encountered and resolved
            if "error" in asst_lower or "failed" in asst_lower or "bug" in asst_lower:
                if any(w in asst_lower for w in ["solucionado", "arreglado", "corregido",
                                                  "fixed", "funciona", "success"]):
                    extracted.append({
                        "content": f"[ERROR->SOLUCION] {assistant_content[:300]}",
                        "type": "resolucion_error",
                        "peer_ids": [],
                        "confidence": 0.75,
                    })

            # 4. Explicit user preferences (from user message patterns)
            pref_patterns = [
                "me gusta", "no me gusta", "prefiero", "siempre ",
                "nunca ", "odio ", "me encanta", "valoro ",
                "importa ", "necesito ", "quiero que",
            ]
            for pat in pref_patterns:
                if pat in user_lower:
                    extracted.append({
                        "content": f"[PREFERENCIA] {user_content[:200]}",
                        "type": "preferencia",
                        "peer_ids": ["peer_usuario"],
                        "confidence": 0.85,
                    })
            ...
            # 5. Durable facts from tool results (file paths, configs, etc.)
            import re
            durable_patterns = [
                (r'(/root/[\w./-]+)', "path"),
                (r'(/mnt/[cd]/[\w./-]+)', "path"),
            ...
            for pattern, dtype in durable_patterns:
                matches = re.findall(pattern, assistant_content[:1000])
                if matches:
                    for line in assistant_content[:1000].split("\n"):
                        if re.search(pattern, line) and any(kw in line.lower() for kw in
                            ["config", "ruta", "archivo", "path", "endpoint", "puerto"]):
                            extracted.append({
                                "content": f"[DATO_DURABLE] {line.strip()[:200]}",
                                "type": "hecho",
                                "peer_ids": [],
                                "confidence": 0.6,
                            })
            ...
            # Store all extracted memories (cap at 5 per turn)
            for mem in extracted[:5]:
                try:
                    payload = {
                        "content": mem["content"],
                        "type": mem.get("type", "hecho"),
                        "source": "hermes-plugin-sync",
                        "confidence": mem.get("confidence", 0.5),
                        "peer_ids": mem.get("peer_ids", []),
                        "metadata": {
                            "session_id": session_id or "",
                            "extraction_method": "rich_memory_extractor",
                            "timestamp": time.time(),
                        },
                    }
                    resp = self._session.post(
                    ...
                    if resp.status_code in (200, 201):
                        logger.debug("F2.5: Rich memory stored: %s", mem["content"][:50])
                except Exception:
                    pass  # Individual memory failure should not break the loop

        except Exception as e:
            logger.debug("F2.5: Rich memory extraction failed: %s", e)
    def _maybe_record_timeline_event(self, user_content: str, assistant_content: str,
                                      session_id: str = "") -> None:
        """Detect milestones in the conversation and record them in timeline.

        Heuristics for milestone detection:
        - Assistant response mentions specific milestone keywords
        - Assistant response starts with completion markers (✅ [x] etc.)
        - Session start signal (first turn with task/plan language)
        """
        try:
            ...
            milestone_keywords = [
                "completad", "terminad", "implementad", "arreglad", "reparad",
                "✅", "[x]", "✔️", "DONE", "FIXED",
                "funcionando", "listo", "terminado", "completado",
                "migrad", "desplegad", "publicad", "cread",
                "endpoint funcion", "api funcion", "tabla cread",
                "repo cre", "rama cre",
                "F1.", "F2.", "F3.", "F4.", "fix:", "feat:",
            ]

            first_200 = assistant_content[:200].lower()
            is_milestone = any(kw.lower() in first_200 for kw in milestone_keywords)

            # Also check for structured checklists in user messages
            if not is_milestone and user_content:
                user_check = user_content[:300].lower()
                # User sent a completion checklist or progress report
                if ("✅" in user_content or "[x]" in user_content) and len(user_content) > 100:
                    is_milestone = True

            if not is_milestone:
                return

            # Build summary from first relevant line of assistant response
            summary = assistant_content[:200].split("\n")[0].strip()
            if len(summary) > 150:
                summary = summary[:147] + "..."
            ...
            timeline_payload = {
                ...
                "related_memories": [],
                "metadata": {
                    "detected_by": "hermes-plugin-sync_turn",
                    "timestamp": time.time(),
                },
            }

            t_resp = self._session.post(
                "/api/v1/timeline/event",
                json=timeline_payload,
                timeout=PING_TIMEOUT,
            )
            if t_resp.status_code in (200, 201):
                logger.debug("F1.4: Timeline milestone recorded")
            else:
                logger.debug("F1.4: Timeline POST returned %d", t_resp.status_code)

        except Exception as e:
            logger.debug("F1.4: Timeline event failed: %s", e)
 ...
 # -- Hook: per-turn lifecycle ------------------------------------------

    # -- Hook: per-turn lifecycle ------------------------------------------

    def on_turn_start(self, turn_number: int, message: str, **kwargs) -> None:
        """Called at the start of each turn by Hermes memory_manager.

        Uses the user message to warm up the route cache (via queue_prefetch)
        so that prefetch() in the same turn gets a cache hit or a ready result.

        Also triggers deferred memory flush from previous slow responses.
        """
        if self._cron_skipped or not self._session or not message:
            return

        # Flush any deferred memory from previous turns into the prefetch buffer
        with _DEFERRED_LOCK:
            if _DEFERRED_MEMORY_BUFFER:
                # Move deferred content into prefetch_result so it gets injected
                deferred_text = "\n".join(_DEFERRED_MEMORY_BUFFER)
                _DEFERRED_MEMORY_BUFFER.clear()
                with self._prefetch_lock:
                    if self._prefetch_result:
                        self._prefetch_result = deferred_text + "\n" + self._prefetch_result
                    else:
                        self._prefetch_result = deferred_text
                logger.debug("on_turn_start: flushed %d deferred blocks",
                             len(deferred_text.split("\n")))

        # Warm up route cache: trigger a background prefetch so the LRU cache
        # has the result ready when prefetch() is called shortly after.
        # Only do this on turn 2+ (turn 1's prefetch will be fresh anyway).
        if turn_number > 1:
    ...
    def _create_memory(self, content: str, peer_id: str,
        ...
        F3: If Muninn is unreachable, queue in offline buffer for later flush.
        """
        tags = ["hermes-sync"]
        ...
        # F3: If no session, queue offline
        ...
        try:
            ...
            if resp.status_code not in (200, 201):
                logger.debug("Muninn _create_memory returned %d", resp.status_code)
                # Connection issue? Queue offline
                if resp.status_code >= 500:
                    self._queue_offline(payload)
        except Exception as e:
            logger.debug("Muninn _create_memory failed: %s", e)
            self._queue_offline(payload)

    # -- F3: Offline buffer management -------------------------------------

    def _queue_offline(self, payload: Dict[str, Any]) -> None:
        """Queue a memory payload for later flush when Muninn comes back."""
        with self._offline_lock:
            if len(self._offline_buffer) < self._offline_max:
                self._offline_buffer.append(payload)
                logger.debug("F3: Queued memory offline (%d buffered)",
    ...
    def _flush_offline_buffer(self) -> None:
        """Try to flush queued offline memories to Muninn.

        Called when initialize() detects Muninn is back.
        """
        with self._offline_lock:
            if not self._offline_buffer:
                return
            pending = list(self._offline_buffer)
            self._offline_buffer.clear()

        if not self._session:
            # Still down — put them back
            with self._offline_lock:
                self._offline_buffer.extend(pending)
            return
        ...
        for payload in pending:
            try:
                ...
                pass  # Will be lost, but we tried

        if flushed:
            logger.info("F3: Flushed %d/%d offline memories to Muninn",
                        flushed, len(pending))
        if flushed < len(pending):
            logger.debug("F3: %d memories could not be flushed",
                         len(pending) - flushed)
